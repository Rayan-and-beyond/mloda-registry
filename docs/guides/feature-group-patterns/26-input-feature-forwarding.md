...
### A Forwarded Reader Key Can Collide With the Root Group Instead of Being Rejected
...
- Or scope the caller's request with `feature_group=<YourConsumerGroup>` so the resolver never considers the root group for this name.
